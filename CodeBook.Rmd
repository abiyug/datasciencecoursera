...
3. Activity type and volunter numbering descirption
4. Data combination and prepration 
...
The experiments have been carried out with a group of 30 volunteers *(labled 1 to 30)**. Each row identifies the volunteer who performed each of the six activity. The activities are labled as follows:
...
### 4. Data combination and prepration 
When downloading the data the data comes ziped. the first action is to unzip the file.  Since the different files contain different part of the same observation and had a raw variable name, the next step is to provide a recognizable variable name and to assemble the the files into a single table.  We start with changing the variable names for volunteers - and activity, and ket the default for the rest (the rest were also modified later - will be explained).

Start the combination for the test-X/train-X, test-y/train-y, test_subject/train_subject datasets using the ‘rbind’ function, and applied the feature file as a variable name for the combined dataset using the ‘colname’ function.  The next step is to put all the data sets together. The dplyr  bind_cols function was used to achieve that. 
